...
public class StickWeaponTrailEffect : MonoBehaviour
{
    ...
    private void UpdateFrameBuffer()
    {
        ...
        var transformed = transform.TransformDirection(normal);

        var top = transformed * width;
        var bottom = transformed * -width;

        if (emitting)
          stickshotBuffer.Add(new Stickshot(time, transform.position + top, transform.position + bottom));
        Profiler.EndSample();
    }
...
private int UpdateSegments(NativeArray<Vector3> vertices)
{
    ...
    for (int i = 0, j = 1; j < count; ++i, ++j)
    {
        ...
        float ageP0 = time - stickshotP0.timeStamp;
        float alphaP0 = Mathf.Clamp01((duration - ageP0) / duration);
        
        float ageP1 = time - stickshotP1.timeStamp;
        float alphaP1 = Mathf.Clamp01((duration - ageP1) / duration);
        ...
        if (interpolations > 1)
        {
            for (int k = 0; k < interpolations; ++k)
            {
                ...
                // Calculate faded vertices
                vertices[verticesCount++] = center - radius * alphaP0; // bottom
                vertices[verticesCount++] = center; // center
                vertices[verticesCount++] = center + radius * alphaP0; // top
            }
        }
        else
        {
            vertices[verticesCount++] = stickshotP0.center - stickshotP0.radius * alphaP0; // bottom
            vertices[verticesCount++] = stickshotP0.center; // center
            vertices[verticesCount++] = stickshotP0.center + stickshotP0.radius * alphaP0; // top
        }
    }

    {
        ...
        vertices[verticesCount++] = center - radius * Mathf.Clamp01((duration - (time - stickshot.timeStamp)) / duration); // bottom
        vertices[verticesCount++] = center; // center
        vertices[verticesCount++] = center + radius * Mathf.Clamp01((duration - (time - stickshot.timeStamp)) / duration); // top
    }
